...
```bash
inference$ sam local invoke InferenceFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
inference$ sam local start-api
inference$ curl http://localhost:3000/classify_digit
```
...
```bash
inference$ sam logs -n InferenceFunction --stack-name inference --tail
```
...
```bash
aws cloudformation delete-stack --stack-name inference
```
